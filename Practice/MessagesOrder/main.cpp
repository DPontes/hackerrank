...
class Recipient {
 public:
    Recipient() {}
    void recieve(const Message& msg) { messages_.push_back(msg); }
    void print_messages() {
        fix_order();
...
class Network {
 public:
    ...
    // simulates the unpredictable network, where sent messages
    // might arrive in unscpecified order
        random_shuffle(messages.begin(), messages.end());
        for (auto& msg : messages) recipient.recieve(msg);
    }
};
